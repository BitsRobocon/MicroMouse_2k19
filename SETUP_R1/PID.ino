...
void pid() { //The algorithm to be actually used
  ...
  float error_both_past = error_both;
  error_both = getDistance(3) - getDistance(2);
  float ed = error_both - error_both_past;

  Serial.print("error : ");

  float turnR = constrain(defaultRPM - gainR * error_both, 120 , 255) + gainDR * ed;
  analogWrite(re, turnR);
  float turnL = constrain(defaultRPM + gainL * error_both, 120, 255) - gainDL * ed;
